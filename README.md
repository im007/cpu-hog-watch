...
The per-process watcher runs anywhere. **The thermal backstop needs
hwmon sensors that vary by hardware** — the defaults read AMD package
temperature (`k10temp`) and the ChromeOS EC (`cros_ec`) found in
Framework laptops and Chromebooks. On other hardware set `THERM_HWMON`,
`THERM_LABEL` and `FAN_HWMON`; see [Configuration](#configuration). If
neither sensor resolves, `--status` and `install.sh --verify` both say
so.
...
This links both scripts into `~/.local/bin`, writes a default config to
`~/.config/cpu-hog-watch.env`, installs a systemd user timer, and
starts it.
...
Your session, your window and your other tabs keep running, and the
dead tab shows a **Restore This Tab** button.
...
site, so two tabs on the same site can share a process, and one tab
with cross-origin frames can span several.
...
| `THERM_HWMON` | `k10temp` | hwmon device carrying package temperature |
| `THERM_LABEL` | `Tctl` | Label of the temperature input to read |
| `FAN_HWMON` | `cros_ec` | hwmon device carrying fan speed |

Common sensor settings for other hardware — run `sensors` to see what a
machine exposes:

| Hardware | Setting |
|---|---|
| Intel CPUs | `THERM_HWMON=coretemp`, `THERM_LABEL="Package id 0"` |
| ThinkPad fans | `FAN_HWMON=thinkpad` |
| Dell fans | `FAN_HWMON=dell_smm` |
...
The per-process watcher examines **only processes owned by the user
running it**. A desktop session cannot signal a root process, so a
runaway system daemon will not raise a per-process alert. The thermal
backstop is what covers that case, since it measures heat regardless of
who owns the cause.
...
|---|---|
| No alerts ever | Check the timer: `systemctl --user status cpu-hog-watch.timer` |
| No thermal alerts | `cpu-hog-watch --status` — if `sensors` reads INACTIVE, set `THERM_HWMON` / `FAN_HWMON` |
| Alerts too frequent | Raise `HOG_STRIKES`, or add the process to `HOG_ALLOW_RE` |
...
| `cpu-hog-watch --status` | Show thresholds, sensors, and anything striking |
...
- **CPU count comes from `/proc/stat`, not `nproc`.** `nproc` honours
  ...
  pegged core reads 6%. Counting `^cpu[0-9]` lines in `/proc/stat`
  keeps the numerator and denominator from disagreeing.
...
- **hwmon devices are resolved by their `name` file.** The numbering
  under `/sys/class/hwmon` is not stable across boots.
...
You may use, modify and run this commercially. The licence requires
reciprocity: if you distribute a modified version — **or run one as a
network service** — you must release your source under the same terms.
That network clause is what distinguishes AGPL from plain GPL.
